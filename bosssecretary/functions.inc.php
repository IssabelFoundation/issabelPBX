        ...
        if (!empty($groups))
        {
            ...
            foreach ($groups as $group)
            {
                ...
                foreach ($group["bosses"] as $extension)
                {
                    ...
                    $ext->add($ctx_bsc, $extension, '', new ext_gosub('1','s',"sub-dial", $args));
...
function bosssecretary_listall_for_rnav() {
    global $db;
    $sql = "SELECT boss_extension AS extension, 'boss' as `type`, g.* FROM bosssecretary_group as g INNER JOIN bosssecretary_boss as b ON b.id_group = g.id_group 
            UNION 
            SELECT secretary_extension AS extension, 'secretary' as `type`, g.* FROM bosssecretary_group as g INNER JOIN bosssecretary_secretary as s ON s.id_group = g.id_group
            UNION
            SELECT chief_extension AS extension, 'chief' as `type`, g.* FROM bosssecretary_group as g INNER JOIN bosssecretary_chief as c ON c.id_group = g.id_group";
...
{
    global $db;
    $sql = "DELETE FROM bosssecretary_group_numbers_free WHERE group_number NOT IN (SELECT id_group FROM bosssecretary_group) AND group_number > (SELECT MAX(id_group) FROM bosssecretary_group)";
    sql($sql);
    $sql = "DELETE FROM bosssecretary_group_numbers_free WHERE group_number = '$number'";
    return sql($sql);
}
...
function bosssecretary_create_nav_groups_links($groups)
{
    $links = array();
...
function bosssecretary_get_form_add( array $params)
{
    ...
    $vars['toast_and_submit'].="</script></div>";
    $vars['toast_and_submit'].=form_action_bar($params["group_number"]);
...
function bosssecretary_show_nav_users($links){
    ...
    $results = bosssecretary_listall_for_rnav();
    $extensions=array();
    foreach($results as $row) {
        $extensions[$row['label']][]=$row['extension'];
    }

    foreach ($links as $link) {
        $hidden_members = implode(",",$extensions[$link['name']]);

        $rnaventries[] = array($link['id'],$link['name']."<span class='is-hidden'>".$hidden_members."</span>",'');
    }
    $display='bosssecretary';
    $type='';
