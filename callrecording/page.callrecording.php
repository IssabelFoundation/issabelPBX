...
echo '<li><a class="'.($extdisplay=='' ? 'current':'').'" href="config.php?display=callrecording&amp;type='.$type.'">'._('Add Call Recording').'</a></li>';

foreach (callrecording_list() as $row) {
	echo '<li><a class="'.($extdisplay==$row['callrecording_id'] ? 'current':'').'" href="config.php?display=callrecording&amp;type='.$type.'&amp;extdisplay='.$row['callrecording_id'].'" class="">'.$row['description'].'</a></li>';
}
