...
$table->add_row(array( 'data' => ipbx_label('Global Pin:', 'A numeric identifier that, depending on the setting of User List and Config Authorization requirements, can be used to assign the configuration of any phone.'), 'class' => 'input'),
	array( 'data' => '<input type="text" id="globalpin" name="globalpin" value="' . $digium_phones->get_general('globalpin') . '" placeholder="Global Pin"/>')
	);

$table->add_row(array( 'data' => ipbx_label('Require Global PIN for user list?:', 'Defines whether the Global PIN is required in order to pull a listing of available phone configurations. Defaults to "No," meaning any phone can pull a listing of all other available phone configurations.')),
...
$table->add_row(array( 'data' => ipbx_label('Phone Authentication Method:', 'This is the authentication type required for a user to take ownership of a phone. By default, no authentication is required and any user can claim any phone configuration. Other available options are: Phone PIN - user must enter the phone\'s PIN in order to claim the phone; Phone MAC Address - the phone\'s actual MAC address must match the configured MAC address for the phone; Global PIN - the user must enter the Global PIN in order to claim the phone.')),
	array( 'data' => $config_auth));

$table->add_row(array( 'data' => ipbx_label('Internal Phonebook Sort Order:', 'Defines the order in which internal phonebooks are sorted.  Default is by extension.')),
