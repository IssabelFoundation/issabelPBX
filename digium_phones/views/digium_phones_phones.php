...
$table->add_row(array( 'data' => ipbx_label('Phone Name:', 'A named identifier for the phone, such as the person using it.')),
				array( 'data' => '<input type="text" id="devicename" name="devicename" />'));
$table->add_row(array( 'data' => ipbx_label('Phone PIN:', 'A numeric identifier associated with this phone. When set, and when enabled in the General Settings page, one must enter this PIN on the phone in order to use this configuration.')),
				array( 'data' => '<input type="text" id="pin" name="pin" />'.$pin_voicemail));
$table->add_row(array( 'data' => ipbx_label('Phone MAC Address:', 'When set, and when enabled in the General Settings page, the phone configuration is only available to the device matching this MAC Address.')),
...
echo ipbx_label('Available Extensions', 'Displays IssabelPBX extensions that may be assigned to a phone. Extensions that are greyed out are in use by another phone already and may not be re-assigned without being first unassigned.');
...
echo '<div class="dragdrop">';
echo ipbx_label('Assigned Extensions', 'Displays a listing of extensions, ordered by Line number, beginning with the first line, assigned to the phone.');
...
echo '<div class="dragdropFrame">';
echo '<div class="dragdrop">';
echo ipbx_label('Available Phonebooks', 'Displays a listing of phonebooks that may be assigned to the Phone\'s Contacts lists. More than one phonebook may be assigned to a phone.');
...
echo '</div>'."\n";
echo '<div class="dragdrop">';
echo ipbx_label('Assigned Phonebooks', 'Displays a listing of phonebooks currently assigned to a Phone. By default, an Internal Phonebook, containing a listing of all system extensions, is assigned to the phone.');
...
$rapiddial .= '</select>';
$table->add_row(array( 'data' => ipbx_label('Rapid Dial Key Phonebook:', 'Sets which Phonebook will be assigned to a Phone\'s Rapid Dial (BLF) keys.')),
				array( 'data' => $rapiddial));
$table->add_row(array( 'data' => ipbx_label('Name Format:', 'Sets the name format used for Rapid Dial keys and Contacts application. Defaults to "FirstName LastName" but may be changed to "LastName, FirstName.')),
...
$tz .= '</select>';
$table->add_row(array( 'data' => ipbx_label('Timezone:', 'Sets the timezone to be used for the phone.')),
				array( 'data' => $tz));
...
echo '<div class="dragdropFrame">';
echo '<div class="dragdrop">';
echo ipbx_label('Available Networks', 'Displays a listing of networks that may be assigned to the phone. More than one network may be assigned to a phone.');
...
echo '</div>'."\n";
echo '<div class="dragdrop">';
echo ipbx_label('Assigned Networks', 'Displays a listing of networks currently assigned to a phone. By default, a Default Network is assigned to the phone.');
...
echo '<div class="dragdropFrame">';
echo '<div class="dragdrop">';
echo ipbx_label('Available Logos', 'Displays a listing of logos that may be assigned to the phone. More than one logo may be assigned to a phone.');
...
echo '</div>'."\n";
echo '<div class="dragdrop">';
echo ipbx_label('Assigned Logos', 'Displays a listing of logos currently assigned to a phone.');
...
echo '<div class="dragdropFrame">';
echo '<div class="dragdrop">';
echo ipbx_label('Available Alerts', 'Displays a listing of alerts that may be assigned to the phone. More than one alert may be assigned to a phone.');
...
echo '</div>'."\n";
echo '<div class="dragdrop">';
echo ipbx_label('Assigned Alerts', 'Displays a listing of alerts currently assigned to a phone.');
...
echo '<div class="dragdropFrame">';
echo '<div class="dragdrop">';
echo ipbx_label('Available Ringtones', 'Displays a listing of ringtones that may be assigned to the phone. More than one ringtone may be assigned to a phone.');
...
echo '</div>'."\n";
echo '<div class="dragdrop">';
echo ipbx_label('Assigned Ringtones', 'Displays a listing of ringtones currently assigned to a phone.');
...
if (!function_exists('presencestate_list_get')) {
	...
	echo '<div class="dragdropFrame">';
	echo '<div class="dragdrop">';
	echo ipbx_label('Available Statuses', 'Displays a listing of statuses that may be assigned to the phone. More than one status should be assigned to a phone.');
	...
	echo '</div>'."\n";
	echo '<div class="dragdrop">';
	echo ipbx_label('Assigned Statuses', 'Displays a listing of statuses currently assigned to a phone.');
...
echo '<div class="dragdropFrame">';
echo '<div class="dragdrop">';
echo ipbx_label('Available Custom Apps', 'Displays a listing of custom applications that may be assigned to the phone. More than one custom application may be assigned to a phone.');
...
echo '</div>'."\n";
echo '<div class="dragdrop">';
echo ipbx_label('Assigned Custom Apps', 'Displays a listing of custom applications currently assigned to a phone.');
...
$table->add_row(array( 'data' => ipbx_label('Enable Call Recording:', 'Enables or Disables Call Recording. If disabled, the Record softkey will not show for in-progress calls.')),
...
$table->add_row(array( 'data' => ipbx_label('Enable Send to Voicemail:', 'Enables or Disables the Send VM softkey. If disabled, the Send VM softkey will not show for incoming calls.')),
...
$table->add_row(array( 'data' => ipbx_label('Require Pin for Voicemail:', 'Enables or Disables requiring the Phone PIN to access Voicemail. If disabled, Voicemail messages can be viewed and played without entering the PIN first.')),
...
$table->add_row(array( 'data' => ipbx_label('Rapid Dials on Unused Line Keys:', 'By default, Line keys that do not have an extension assigned to them will be configured with Rapid Dial keys. This behavior may be disabled such that Rapid Dial keys begin assignment on the sidecar keys.')),
...
$table->add_row(array( 'data' => ipbx_label('Select Firmware:', 'Pick the firmware to load on the phone.')),
				array( 'data' => $firmware));
...
$table->add_row(array( 'data' => ipbx_label('Active Locale:', 'Set the default active locale')),
...
$table->add_row(array( 'data' => ipbx_label('Default Ringtone:', 'Sets the default ringtone for a phone. In order to play a custom Ringtone by default, that Ringtone must be loaded onto a phone using the Ringtones option.')),
				array( 'data' => $ringtoneOptions));
	
$table->add_row(array( 'data' => ipbx_label('Phone Admin Password:', 'Sets a custom password for the phone\'s web interface, if enabled. Note that the web interface is disabled by default, as its use can run in conflict to use of the phones with the DPMA and result in unexpected behavior. Do not enable the phone\'s web interface unless you know what you\'re doing.')),
				array( 'data' => '<input type="text" id="login_password" name="login_password"/>'));
				
$table->add_row(array( 'data' => ipbx_label('Accept Only Local Calls:', 'Sets whether the phone will accept calls only from the PBX, or instead from any address.')),
...
$table->add_row(array( 'data' => ipbx_label('Enable Call Waiting Tone:', 'If disabled, the phone will not play a call waiting tone when it receives a new call while already on a call.')),
...
$table->add_row(array( 'data' => ipbx_label('Brightness Level:', 'Sets the LCD screen brightness, defaults to 5.')),
...
$table->add_row(array( 'data' => ipbx_label('Contrast Level:', 'Sets the LCD screen contrast, defaults to 5.')),
...
$table->add_row(array( 'data' => ipbx_label('Dim Backlight After Timeout:', 'Enables backlight dimming. When enabled, dims the screen after backlight timeout has been reached and phone is otherwise idle. Defaults to enabled.')),
...
$table->add_row(array( 'data' => ipbx_label('Backlight Dim Level:', 'Sets the brightness level to which the phone dims when when Dim Backlight After Timeout is enabled, defaults to 2.')),
...
$table->add_row(array( 'data' => ipbx_label('Backlight Timeout:', 'Sets the time, in seconds, before the backlight is set to Backlight Dim Level while phone is idle; setting to 0 disables backlight timeout, defaults to 0.')),
				array( 'data' => '<input type="text" id="backlight_timeout" name="backlight_timeout"/>'));
				
$table->add_row(array( 'data' => ipbx_label('Default Font Size:', 'Sets the default font size for the phone. Caution should be exercised when using this option as larger sizes will cause labels to overrun their allowed space. D40, D45 and D50 default to 10. D70 defaults to 11.')),
                                array( 'data' => '<input type="text" id="default_fontsize" name="default_fontsize"/>'));

$table->add_row(array( 'data' => ipbx_label('Display Missed Call Notifications:', 'Defines whether the phone will display a notification for missed calls or not.')),
...
$table->add_row(array( 'data' => ipbx_label('Ringer Volume:', 'Sets the ringer volume, defaults to 5.')),
...
$table->add_row(array( 'data' => ipbx_label('Speaker Volume:', 'Sets the speaker volume, defaults to 5.')),
...
$table->add_row(array( 'data' => ipbx_label('Handset Volume:', 'Sets the handset volume, defaults to 5.')),
...
$table->add_row(array( 'data' => ipbx_label('Headset Volume:', 'Sets the headset volume, defaults to 5.')),
...
$table->add_row(array( 'data' => ipbx_label('Call Volume Persistent Across Calls:', 'If enabled, volume changes made during a call do not persist to the next call, defaults to disabled.')),
...
$table->add_row(array( 'data' => ipbx_label('Prefer Handset to Headset:', 'Sets whether to use the headset, rather than the speaker, for answering all calls, defaults to disabled.')),
