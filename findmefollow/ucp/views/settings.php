...
<form role="form">
	...
	<div class="form-group">
		<label for="follow_me_list"><?php echo __('Follow Me List')?></label>
		<textarea id="follow_me_list" name="follow_me_list" class="form-control" rows="3"><?php echo implode("\n",$list)?></textarea>
	</div>
	<div class="form-group">
		<label for="follow_me_prering_time"><?php echo sprintf(__('Ring %s First For'),$exten) ?>:</label><br/>
		<select name="follow_me_prering_time" id="follow_me_prering_time">
			<?php foreach($prering_time as $key => $value) { ?>
				<option value="<?php echo $key?>" <?php echo ($prering == $key) ? 'selected' : ''?>><?php echo $value?> <?php echo __('Seconds')?></option>
			<?php } ?>
		</select>
	</div>
	<div class="form-group">
		<label for="follow_me_listring_time"><?php echo __('Ring Followme List For') ?>:</label><br/>
		<select name="follow_me_listring_time" id="follow_me_listring_time">
			<?php foreach($listring_time as $key => $value) { ?>
				<option value="<?php echo $key?>" <?php echo ($ringtime == $key) ? 'selected' : ''?>><?php echo $value?> <?php echo __('Seconds')?></option>
			<?php } ?>
		</select>
	</div>
	<label class="envelope">
		<?php echo __('Use Confirmation')?>
		<div class="onoffswitch">
