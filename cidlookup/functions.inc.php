...
//  Copyright 2024 Issabel Foundation
            ...
            if(is_array($sources)) {
                ...
                foreach($sources as $item) {
                    ...
                    switch($item['sourcetype']) {
                        ...
                        case "internal":
                            if($item['setvariable']=='cidname') {
                                $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_lookupcidname(''));
                            } else {
                                $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_execif('$["${DB(cidname/${CALLERID(num)})}" != ""]', 'Set', 'CDR(userfield)=${DB(cidname/${CALLERID(num)})}'));
                            }
                        break;
                        ...
                        case "enum":
                            if($item['setvariable']=='cidname') {
                                ...
                                $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_setvar('CDR(userfield)', '${TXTCIDNAME(${CALLERID(num)})}'));
                            }
                        break;
                        ...
                        case "opencnam":
                            ...
                            if($item['setvariable']=='cidname') {
                                ...
                                $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_set('CDR(e)', $curl));
                            }
                        ...
                        case "http":
                            ...
                            if ($ast_ge_162) {
                                ...
                                if($item['sourcetype']=='https') {
                                    $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_set('CURLOPT(ssl_verifypeer)', 'false'));
                                }
                            }
                            if($item['setvariable']=='cidname') {
                                $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_set('CALLERID(name)', $curl));
                            } else {
                                $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_set('CDR(userfield)', $curl));
                            }
                        break;
                        ...
                        case "mysql":
                          if (version_compare($version, "1.6", "lt")) {
                            ...
                            if($item['setvariable']=='cidname') {
                                ...
                                $ext->add('cidlookup', 'cidlookup_'.$item['cidlookup_id'], '', new ext_mysql_fetch('fetchid', 'resultid', 'CDR(userfield)'));
                            }
...
function cidlookup_add($post){
    ...
    $setvariable = $db->escapeSimple($post['setvariable']);
    ...
    $results = sql("
        INSERT INTO cidlookup
            (description, sourcetype, cache, deptname, http_host, http_port, http_username, http_password, http_path, http_query, mysql_host, mysql_dbname, mysql_query, mysql_username, mysql_password, mysql_charset, opencnam_account_sid, opencnam_auth_token, setvariable)
        VALUES
            ('$description', '$sourcetype', '$cache', '$deptname', '$http_host', '$http_port', '$http_username', '$http_password', '$http_path', '$http_query', '$mysql_host', '$mysql_dbname', '$mysql_query', '$mysql_username', '$mysql_password', '$mysql_charset', '$opencnam_account_sid', '$opencnam_auth_token', '$setvariable')
        ");
}
