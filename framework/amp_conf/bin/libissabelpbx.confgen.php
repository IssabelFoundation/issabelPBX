...
function generate_configurations_sip($ast_version) {
	...
	$warning_banner = __("; do not edit this file, this is an auto-generated file by issabelpbx\n; all modifications must be done from the web gui\n\n");
	...
	if ($sip_conf_fh === false) {
		fatal(__("Cannot write SIP configurations"),sprintf(__("Failed creating/overwriting SIP extensions file: %s"),$sip_conf));
	}
	$sip_reg_fh = fopen($sip_reg,"w");
	if ($sip_reg_fh === false) {
		fatal(__("Cannot write SIP registrations"),sprintf(__("Failed creating/overwriting SIP registrations file: %s"),$sip_reg));
	}
...
function generate_configurations_iax($ast_version) {
	...
	if ($iax_conf_fh === false) {
		fatal(__("Cannot write IAX configurations"),sprintf(__("Failed creating/overwriting IAX extensions file: %s"),$iax_conf));
	}
	$iax_reg_fh = fopen($iax_reg,"w");
	if ($iax_reg_fh === false) {
		fatal(__("Cannot write IAX registrations"),sprintf(__("Failed creating/overwriting IAX registrations file: %s"),$iax_reg));
	}
...
function generate_configurations_zap($ast_version) {
	...
	if ($zap_conf_fh === false) {
		fatal(__("Cannot write $channel_name configurations"),sprintf(__("Failed creating/overwriting $channel_name extensions file: %s"),$zap_conf));
	}
...
function generate_configurations_queues($ast_version) {
	...
	if ($queues_conf_fh === false) {
		fatal(__("Cannot write Queues configurations"),sprintf(__("Failed creating/overwriting Queues extensions file: %s"),$queues_conf));
	}
