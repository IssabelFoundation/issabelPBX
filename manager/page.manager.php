...
?>
<!--div class="rnav"><ul>
    <li><a class="<?php echo ($extdisplay=='' ? 'current':'') ?>" href="config.php?type=tool&amp;display=<?php echo urlencode($dispnum)?>"><?php echo _("Add Manager")?></a></li>
<?php
if (isset($managers)) {
	foreach ($managers as $manager) {
		echo "<li><a class=\"".($extdisplay==$manager['name'] ? 'current':'')."\" href=\"config.php?type=tool&amp;display=".urlencode($dispnum)."&extdisplay=".$manager['name']."\">{$manager['name']}</a></li>";
	}
}
?>
<ul></div-->
...
/*
if ($action == 'delete') {
	echo '<br><h3>'._("Manager").' '.$extdisplay.' '._("deleted").'!</h3><br><br><br><br><br><br><br><br>';
...
<?php		if ($extdisplay){ ?>
	<h2><?php echo _("Edit Manager").": ". $extdisplay; ?></h2>
<?php		} else { ?>
...
?>
	...
	$('#rallnone').on('change',function() {
		$('.rpermission').prop('checked',$(this).is(':checked'));
	});

	$('#wallnone').on('change',function() {
