...
if ($extdisplay != "") {
	echo "<h2>".sprintf(__("DUNDi Information: %s"),$extdisplay)."</h2>";
	if ($dundiconflict == 'true') {
		echo sprintf(__("The number you are trying to use, %s, is currently available from one of the DUNDi routes you have configured on your system. As a result you cannot use this number on this system. Even if the route configuration does not pass this number you will still be blocked from creating it. If this is not an error, then you will have to un-publish this number on your remote DUNDi setup, disable the DUNDi trunk in question, or disable this module to avoid the checks. Otherwise, remove %s from the remote system before creating this one."),$extdisplay,$extdisplay);
	}
	$list = dundicheck_lookup_all($extdisplay);
	if (empty($list)) {
		echo "<div class='notifications box'>".__("No matches found")."</div>";
	} else {
		foreach ($list as $map => $line) {
			echo "<div class='notifications box'>".sprintf(__("Results from DUNDi trunk: %s"),$map)."</div>";
			$output = explode("\n",$line);
			unset($output[0]);
...
<?php
	...
	echo "<h2>".sprintf(__("DUNDi Lookup"))."</h2>";
}
...
<form name="dundicheck" id='mainform' method="post">
	...
	<table class='table is-borderless is-narrow notfixed'>
		<tr>
			<td><label><?php echo ($extdisplay == '')?__("Lookup Number"):__("Lookup Another Number")?></label>
			<input name="extdisplay" type="text" class="input" value="<?php htmlspecialchars($extdisplay);?>"></td>
            <td valign="top">
                <br/>
				<input type="submit" class="button" value="<?php echo __("Lookup")?>">
			</td>
		</tr>
