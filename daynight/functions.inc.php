...
function daynight_edit($post, $id=0) {
    ...
    $day   = isset($post[$post['goto0']])?$post[$post['goto0']]:'';
    $night = isset($post[$post['goto1']])?$post[$post['goto1']]:'';
...
function daynight_edit_timecondition($viewing_extdisplay, $daynight_ref) {
    global $db;

    $sql = "DELETE FROM `daynight` WHERE `dmode` IN ('timeday', 'timenight') AND dest = '$viewing_extdisplay'";
    $res = $db->getAll($sql, DB_FETCHMODE_ASSOC);

    if ($daynight_ref != '') {
        $daynight_vals = explode(',',$daynight_ref,2);
        $sql = "INSERT INTO `daynight` (`ext`, `dmode`, `dest`) VALUES ('".$daynight_vals[0]."', '".$daynight_vals[1]."', '$viewing_extdisplay')";
        sql($sql);
    }
}
...
function daynight_checkadd_timecondition() { 
    ...
    if (! empty($res)) {
        ...
        $viewing_extdisplay = $timeconditions_ids[0];

        $sql = "UPDATE `daynight` SET `dest` = '$viewing_extdisplay' WHERE `dest` = 'add'";
        sql($sql);
    }
}

function daynight_del_timecondition($viewing_extdisplay) {
    global $db;
...
//
function daynight_hook_timeconditions($viewing_extdisplay, $target_menuid) {
    global $tabindex;
    global $amp_conf;
    switch ($target_menuid) {
        // only provide display for timeconditions
        case 'timeconditions':
            $current = daynight_get_timecondition($viewing_extdisplay);
...
function daynight_hookProcess_timeconditions($viewing_extdisplay, $request) {
    ...
    if(!isset($request['action']) ) {
        ...
        switch ($request['action'])    {
            case 'add':
                // we don't have an viewing_extdisplay at this point
                daynight_add_timecondition($daynight_ref);
                break;
            case 'delete':
                daynight_del_timecondition($viewing_extdisplay);
                break;
            case 'edit':
                daynight_edit_timecondition($viewing_extdisplay, $daynight_ref);
                break;
        }
    }
}
