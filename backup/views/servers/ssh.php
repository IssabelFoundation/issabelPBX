...
if($id=='') {
    $html .= heading(__('Add SSH Server'), 2);
} else {
    $html .= heading(__('Edit SSH Server').": ".$name, 2);
}
...
//hostname
$label = ipbx_label(__('Hostname'), __('IP address or FQDN of remote ssh host'));
$data  = array(
			'name' => 'host', 
			'value' => $host,
			'required' => '',
...
$table->add_row(ipbx_label(__('Port'), __('remote ssh port')), form_input($data));
...
//ssh key
$label	= ipbx_label(__('Key'), __('Location of ssh private key to be used when connecting to a host'));
$data 	= array(
			'name' => 'key', 
			'value' => $key,
			'required' => '',
...
//remote directory
$label	= ipbx_label(__('Path'), __('Path on server where files are stored'));
$data 	= array(
			'name' => 'path', 
			'value' => $path,
            'class'=>'input'
