...
$helptext = _("Each Balanced Trunk is an outbound trunk associated with a set of parameters to define the maximum use you want to do with it. For instance you have a provider that gives you 100 minutes long distance calls per month. You can define here that after 100 minutes of local call during the month this trunk will become unavailable and your route will switch to the next trunk in line.");
...
?>
<div class='is-flex'><h2><?php echo ($extdisplay ? _("Edit Balanced Trunk")." ". $extdisplay : _("Add Balanced Trunk")); ?></h2><?php echo $help;?></div>
...
function edit_onsubmit(theForm) {
	if (isEmpty(theForm.description.value)) return warnInvalid(theForm.description, "<?php echo _("Please enter a valid Description") ?>");
	if (!isAlphanumeric(theForm.description.value)) return warnInvalid(theForm.description, "<?php echo _("Please enter a valid Description") ?>");
    if ((theForm.desttrunk_id.value)=="0") return warnInvalid($('#desttrunk_id')[0], "<?php echo _("Please select a valid trunk")?>");
    $.LoadingOverlay('show');
	return true;
}
